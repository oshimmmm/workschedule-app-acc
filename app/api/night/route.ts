 ...
 *  6. スタッフ選定時は、選択された部門を持つかつ配置するポジション名（pos.name）も departments に持つスタッフのみ対象
 */
async function assignNightShift(
  ...
  for (const pos of positions) {
    ...
    // eligibleStaff：該当部門＋pos.nameを departments に持つスタッフ
    // prefer-const 対応
    ...
    if (eligibleStaff.length === 0) {
      console.log(
        `[${formatDateLocal(date)}] Position: ${pos.name} - No eligible staff available for department ${selectedDept}`
      );
      assignments[pos.id] = "未配置";
    } else {
      ...
      console.log(
        `[${formatDateLocal(date)}] Position: ${pos.name} - Updated staff counter for ${
          chosenStaffInfo.key
        }: ${staffCounters.get(chosenStaffInfo.key)}`
      );
...
async function assignNichokuShift(
  ...
  for (const pos of positions) {
    ...
    const eligibleStaff = staffList.filter(
      (staff) =>
        (staff.departments || []).includes(selectedDept!) &&
        (staff.departments || []).includes(pos.name)
    );
    if (eligibleStaff.length === 0) {
      console.log(
        `[${formatDateLocal(date)}] Nichoku Position: ${pos.name} - No eligible staff available for department ${selectedDept}`
      );
      assignments[pos.id] = "未配置";
    } else {
      const candidateStaffInfo: { staff: StaffData; key: string; count: number }[] = [];
      ...
      console.log(
        `[${formatDateLocal(date)}] Nichoku Position: ${pos.name} - Updated staff counter for ${
          chosenStaffInfo.key
        }: ${staffCounters.get(chosenStaffInfo.key)}`
      );
      assignments[pos.id] = selectedStaff.name;
  ...
  for (const monthStr of monthRange) {
    ...
    for (const date of dates) {
      ...
      while (attempts < MAX_ATTEMPTS) {
        ...
        // 夜勤割当
        const nightAssignments = await assignNightShift(
          date,
          nightPositions,
          staffList,
          counters,
        ...
        if (currentIsHoliday && nichokuPositions.length > 0) {
          const nichokuAssignments = await assignNichokuShift(
            date,
            nichokuPositions,
            staffList,
            counters,
